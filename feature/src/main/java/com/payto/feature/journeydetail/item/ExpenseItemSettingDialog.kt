...
@Composable
fun ExpenseSettingBottomSheetDialog(
    modifier: Modifier,
    isShow: Boolean,
    onDismissRequest: (ExpenseSettingType?) -> Unit,
) {

    val sheetState = rememberModalBottomSheetState()
    val coroutineScope = rememberCoroutineScope()

    val onSelected: (ExpenseSettingType?) -> Unit = { setting ->
        coroutineScope
...
private fun Content(
    ...
    onSelected: (ExpenseSettingType) -> Unit
) {
    val orderList = remember {
        ExpenseSettingType.entries.toList()
    }
    Column(
...
private fun SettingItem(
    setting: ExpenseSettingType,
    onClick: (ExpenseSettingType) -> Unit
) {
    Row(
        modifier = Modifier
        ...
        Text(
            ...
            color = if (setting == ExpenseSettingType.DELETE) Color.Status.error else Color.Label.neutral
        )
    }
}
